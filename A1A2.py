...
reviews = '''USING PERIODIC COMMIT LOAD CSV WITH HEADERS FROM ''' + '''"{0}{1}"'''.format(path, file7) + ''' AS row MATCH (paper:Paper {paperID: row.paperID}) MATCH (author:Author {authorID: row.authorID})
MERGE (paper)<-[:REVIEWS]-(author);'''

contains = '''USING PERIODIC COMMIT LOAD CSV WITH HEADERS FROM ''' + '''"{0}{1}"'''.format(path, file8) + ''' AS row MATCH (paper:Paper {paperID: row.paperID}) MATCH (kw:Keyword {keywordID: row.keywordID})
MERGE (paper)-[:CONTAINS]->(kw);'''
